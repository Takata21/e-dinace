...
export function Aside({ isVisible, setVisible }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const maxInterval = 2 * 365 * 24 * 60 * 60 * 1000 // 2 años en milisegundos

    if (endDate - startDate > maxInterval) {
      alert('El intervalo de fechas no puede ser mayor a 2 años.')
      return
    }
  ...
  return (
    <div className="flex items-center justify-center">
      ...
      <aside
        className={clsx(
          {
            'invisible hidden top-[9999px] h-0': isVisible !== true,
          },
          'bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700 overflow-hidden position absolute top-0  bottom-0 left-0 right-0 z-50 h-full transition duration-1000 lg:'
        )}
      >
        <form className="px-5 py-3" onSubmit={handleSubmit}>
          <div className="">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                className="flex items-center justify-center w-8 h-8 mb-2 text-sm font-medium text-white bg-red-700 rounded-lg focus:outline-none hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"
                onClick={() => setVisible()}
              >
                <IoMdClose size={25} />
              </button>
